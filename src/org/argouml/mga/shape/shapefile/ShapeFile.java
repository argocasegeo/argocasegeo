...
public class ShapeFile {
    ...
    private RandomAccessFile /*DataOutputStream*/ SHPWriter, SHXWriter;
    ...
    /**
     * Cria um novo arquivo shape
     * @param shapeType_ indica o tipo de objeto armazenado no arquivo
     * @param name nome do arquivo armazenado
     * @param path caminho onde o arquivo sera armazenado
     */
    ...
    public boolean open( String path, String name ) {
        DBFField DBFField_;         // classe Shapes
        DBField DBField_;           // classe DBFUnit
        ...
        try {
            ...
            shapeType = invByteOrder( BRi( "SHP" ) );           // Shape Type
            box.setXMin( invByteOrder( BRr( "SHP" ) ) );        // Coord. Box.XMin
            box.setYMin( invByteOrder( BRr( "SHP" ) ) );        // Coord. Box.YMin
            box.setXMax( invByteOrder( BRr( "SHP" ) ) );        // Coord. Box.XMax
            box.setYMax( invByteOrder( BRr( "SHP" ) ) );        // Coord. Box.YMax
            
            // Carregar estrutura do DBF
            if( DBFFile_.load() ) {
                Vector dbFields = new Vector( DBFFile_.getFields() );
                for( int nf = 0; nf < dbFields.size(); nf++ ) {
                    DBFField_ = new DBFField();
                    fields.add( DBFField_ );
                    DBField_ = (DBField)dbFields.get( nf );
                    DBFField_.setNome( DBField_.getName() );
                    DBFField_.setFType( (byte)DBField_.getFType() );
    ...
    public int invByteOrder( int num ) {
        byte [] i = new byte[4];
        i[0] = (byte)( num );
        i[1] = (byte)( num >> 8 );
        i[2] = (byte)( num >> 16 );
    ...
    /*
     * Escreve no arquivo shape um registro do tipo ponto
     * devolve o número de bytes escritos (16 bit word)
     * @param recordNumber_ Numero sequencial do registro
     * @param offSet Numero de bytes ja gravados, ou seja, deslocamento do registro atual
     * @param RC Entidade a ser escrita.
     */
    ...
    /*
     * Escreve no arquivo shape um registro do tipo poligono
     * devolve o número de bytes escritos (16 bit word)
     * @param recordNumber_ Numero sequencial do registro
     * @param offSet Numero de bytes ja gravados, ou seja, deslocamento do registro atual
     * @param RC Entidade a ser escrita.
    ...
    /*
     * Escreve no arquivo shape um registro do tipo polyline
     * devolve o número de bytes escritos (16 bit word)
     * @param recordNumber_ Numero sequencial do registro
     * @param offSet Numero de bytes ja gravados, ou seja, deslocamento do registro atual
     * @param RC Entidade a ser escrita.
    ...
    /*
     * Escreve no arquivo shape um registro do tipo multipontos
     * devolve o número de bytes escritos (16 bit word)
     * @param recordNumber_ Numero sequencial do registro
     * @param offSet Numero de bytes ja gravados, ou seja, deslocamento do registro atual
     * @param RC Entidade a ser escrita.
    ...
    private void addReadPoint() {
        ...
        for( int i = 0; i < RCP.getNumFields(); i++ )
            RCP.setValue( i + 1, DBFFile_.getField( nRegistro, i ) );
    ...
    private void addReadPolyline() {
        ...
        if( nRegistro >= DBFFile_.getNumRecords() ) {
            eof = true;
            return;
        }
        //Recuperar os valores do registro do BD
        ...
        for( int i = 0; i < numParts; i++ ) {
            int unUsed = BRi( "SHP" );          // Jah que o valor nao serah usado
                                                // o procedimento de inversao da
                                                // ordem dos bytes nao foi usado
        }
    ...
    public boolean save() {
        ...
        try {
            SHPFile = new File( filePath + fileName + ".shp" );
            SHPWriter = new RandomAccessFile( SHPFile, "rw" ); 
            SHXFile = new File( filePath + fileName + ".shx" );
            SHXWriter = new RandomAccessFile( SHXFile, "rw" ); 
            ...
            if( records.size() == 0 )
                DBFFile_.insert();
            SHPWriter.setLength( SHPFL * 2 );
            SHXWriter.setLength( SHXFL * 2 );
