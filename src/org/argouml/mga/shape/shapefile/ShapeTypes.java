/*
 * ShapeTypes.java
 *
 * Created on 14 de Julho de 2004, 17:30
 */

package org.argouml.mga.shape.shapefile;

/**
 *
 * @author  Valerio
 */
public abstract class ShapeTypes {

    public static final int SHP_NULL_SHAPE    =  0;
    public static final int SHP_POINT         =  1;
    public static final int SHP_POLYLINE      =  3;
    public static final int SHP_POLYGON       =  5;
    public static final int SHP_MULTIPOINT    =  8;
    public static final int SHP_POINT_Z       = 11;
    public static final int SHP_POLYLINE_Z    = 13;
    public static final int SHP_POLYGON_Z     = 15;
    public static final int SHP_MULTIPOINT_Z  = 18;
    public static final int SHP_POINT_M       = 21;
    public static final int SHP_POLYLINE_M    = 23;
    public static final int SHP_POLYGON_M     = 25;
    public static final int SHP_MULTIPOINT_M  = 28;
    public static final int SHP_MULTIPATCH    = 31;
    
}
